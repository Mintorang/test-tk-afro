...
import { sendOrderConfirmationEmail, sendKitchenNotificationEmail, sendPaymentFailedEmail } from '@/lib/email-service';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (event.success && event.data) {
      ...
      if (event.data.type === 'account.updated') {
      ...
      } else if (event.data.type === 'payment_intent.payment_failed') {
        const paymentIntent = event.data.data.object as Stripe.PaymentIntent;
        ...
        try {
          ...
          console.log(`Payment failed email sent for PaymentIntent: ${orderId}`);
        } catch (emailError) {
          console.error(`Failed to send payment failed email for PaymentIntent ${orderId}:`, emailError);
        }

        // Log the payment failure for analysis
        console.log('📊 Stripe payment failure logged:', {
          orderId,
          customerEmail,
          amount,
