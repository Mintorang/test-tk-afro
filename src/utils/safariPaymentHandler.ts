...
export class SafariPaymentHandler {
  ...
  private async processStripeWithSafariWorkaround(paymentData: PaymentData) {
    ...
    // For Safari, we recommend using Apple Pay instead
    throw new Error('Stripe payments may have issues in Safari. Please use Apple Pay for the best experience.');
  }
  ...
  private addSafariWarnings() {
    ...
    warningDiv.innerHTML = `
      ...
      ">
        ...
        For the best experience, we recommend using Apple Pay.
      </div>
    `;
  ...
  private handlePaymentError(error: any) {
    ...
    if (this.environment?.isIOSSafari) {
      userMessage += ' If the problem persists, try using Apple Pay, or switch to Chrome.';
    }
