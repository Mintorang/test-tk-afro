...
export default function OptimizedCheckout() {
  ...
  const [deliveryRegion, setDeliveryRegion] = useState<'england-wales' | 'scotland'>('england-wales');
  ...
  // UK delivery fee by region
  const getDeliveryFee = () => (deliveryRegion === 'scotland' ? 29.99 : 24.99);
  ...
  useEffect(() => {
    ...
    if (fulfillmentType === 'delivery') {
      // Calculate next business day delivery (Wednesday-Saturday only)
      let deliveryTime = new Date(now.getTime() + (24 * 60 * 60 * 1000)); // Start with next day
      
      // Adjust for business days (Wednesday-Saturday)
      const dayOfWeek = deliveryTime.getDay(); // 0=Sunday, 1=Monday, 2=Tuesday, 3=Wednesday, 4=Thursday, 5=Friday, 6=Saturday
      
      // If next day is Sunday (0), Monday (1), or Tuesday (2), move to Wednesday
      if (dayOfWeek === 0) { // Sunday
        deliveryTime = new Date(deliveryTime.getTime() + (3 * 24 * 60 * 60 * 1000)); // Add 3 days to get to Wednesday
      } else if (dayOfWeek === 1) { // Monday
        deliveryTime = new Date(deliveryTime.getTime() + (2 * 24 * 60 * 60 * 1000)); // Add 2 days to get to Wednesday
      } else if (dayOfWeek === 2) { // Tuesday
        deliveryTime = new Date(deliveryTime.getTime() + (1 * 24 * 60 * 60 * 1000)); // Add 1 day to get to Wednesday
      }
      ...
      // Collection time (45-60 minutes from now, but only during business hours)
  ...
  const handlePaymentSuccess = async (paymentData: any, method: 'card' | 'bank' | 'mobile' | 'square' | 'openbanking') => {
  ...
  const handlePaymentError = async (error: any, method: 'card' | 'bank' | 'mobile' | 'square' | 'openbanking') => {
    console.error('❌ Payment error in checkout:', { error, method });
    console.error('❌ Payment error details:', JSON.stringify(error, null, 2));
    
    // Send payment failure notification to backend
    try {
      ...
      console.log('✅ Payment failure notification sent to backend');
    } catch (notificationError) {
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm">
                      ...
                      >
                        <div className="flex items-start space-x-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600 mb-3">Next business day delivery anywhere in the UK mainland (Wednesday-Saturday)</p>
                            <div className="space-y-1">
                              <div className="flex items-center text-sm">
                                <span className="text-gray-500">Delivery fee:</span>
                                <span className="font-semibold text-orange-600 ml-2">£{getDeliveryFee().toFixed(2)}</span>
                              </div>
                              <div className="flex items-center text-sm">
                                <span className="text-gray-500">Estimated delivery:</span>
                                <span className="font-medium text-gray-700 ml-2">Next business day</span>
                              </div>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center text-sm">
                                ...
                                <span className="font-medium text-gray-700 ml-2">We will notify you when ready</span>
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {fulfillmentType === 'delivery' && (
                      <>
                        <div>
                          <Label htmlFor="deliveryRegion" className="text-gray-700 font-medium">Delivery Region</Label>
                          <select
                            id="deliveryRegion"
                            value={deliveryRegion}
                            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setDeliveryRegion(e.target.value as 'england-wales' | 'scotland')}
                            className="mt-1 block w-full rounded-md border-gray-400 bg-white text-gray-900 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-base p-3"
                          >
                            <option value="england-wales">England & Wales (£24.99)</option>
                            <option value="scotland">Scotland (£29.99)</option>
                          </select>
                        </div>
                        <div>
                          ...
                          <div className="flex items-start space-x-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-blue-800">
                                <p>We'll email you when your order is ready for collection</p>
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-lg shadow-sm sticky top-8">
                    ...
                    <div>
                      ...
                      <p className={`text-sm ${
                        ...
                        {fulfillmentType === 'delivery' 
                          ...
                          : `${estimatedDelivery} (We will notify you when ready)`
                        }
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-2 border-t border-gray-200 pt-4">
                  ...
                  {fulfillmentType === 'delivery' && (
                    <p className="text-xs text-orange-600">Next business day delivery anywhere in UK mainland (Wednesday-Saturday)</p>
                  )}
                  {fulfillmentType === 'collection' && (
