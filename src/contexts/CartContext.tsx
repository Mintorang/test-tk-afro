...
interface CartContextType {
  ...
  removeFromCart: (itemId: string, size: string, spiceLevel?: string) => void;
  updateQuantity: (itemId: string, size: string, quantity: number, spiceLevel?: string) => void;
  clearCart: () => void;
  totalPrice: number;
  isCartOpen: boolean;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const removeFromCart = (itemId: string, size: string, spiceLevel?: string) => {
    setItems(currentItems => 
  ...
  const updateQuantity = (itemId: string, size: string, quantity: number, spiceLevel?: string) => {
    if (quantity < 1) {
      removeFromCart(itemId, size, spiceLevel);
      return;
    }
